...
void Primes::remove_multiples(std::vector<unsigned int>& nums,
...
std::vector<unsigned int> Primes::create_wheel(
    std::vector<unsigned int>& starter_primes) {
...
std::list<uint64_t> Primes::sieve_between(
    const std::list<uint64_t>& starter_primes, uint64_t start, uint64_t end) {
  ...
  uint64_t n;
  for (auto p = std::next(starter_primes.begin()); p != starter_primes.end();
       ++p) {
    p_double = (*p * 2);
 ...
 * @return
 */
std::vector<bool> Primes::get_2_sieve_between(
    std::list<uint64_t> starter_primes, uint64_t start, uint64_t end) {
...
void Primes::convert_2_sieve_between(std::list<uint64_t>& primes,
                                     const std::vector<bool>& sieve,
                                     uint64_t start) {
  start = start + ((start + 1) % 2);  // Make start odd in the same way that
                                      // Primes::get_2_sieve_between does
...
/*
 * 1 array of uint64_t of sufficient size which holds all of the prime numbers
 * (use pi function) 1 vector of bool
 */
...
#pragma omp parallel for ordered default(none)                  \
    shared(max_value, root_max_value, divisions, division_size, \
           starter_primes, primes, std::cout)
  for (unsigned int i = 0; i < divisions; ++i) {
    ...
    std::list<uint64_t> partial =
        Primes::sieve_between(starter_primes, start, finish);
...
std::vector<uint64_t> Primes::gen_primes(uint64_t max_value) {
  uint64_t primes_array_size =
      uint64_t(double(max_value) / (log(max_value) - 2));
